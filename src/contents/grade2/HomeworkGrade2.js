...
function HomeworkGrade2() {
    ...
    const fetchHomework = async () => {
      try {
        const res = await fetch(
          'https://raw.githubusercontent.com/workcompany177/trash/main/control/grade2/homework.json'
        );
  ...
  return (
    // غلاف يحمل الخلفية المخصصة
    <div
      style={{
        minHeight: '100vh', // لجعل الخلفية تمتد لطول الصفحة
        // مثال لخلفية متدرجة
        background: 'linear-gradient(to bottom right, #b21f1f, #b21f1f, #3a1c71)',
        backgroundSize: 'cover',
        backgroundRepeat: 'no-repeat',
        backgroundAttachment: 'fixed', // ثبات الخلفية أثناء التمرير
      }}
    >
      <Container style={{ direction: 'rtl', color: '#fff', paddingTop: '40px' }}>
        <h2 className="mb-4" style={{ textAlign: 'center' }}>
          واجبات الصف الثاني الثانوي
        </h2>
